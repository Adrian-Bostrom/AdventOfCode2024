...
bool ProblemDampener(int *inputLine) {
    int *inputCopy = calloc(30,sizeof(int));
    int i = 0;
    while(i < 30 && inputLine[i] != 0){
        memcpy(inputCopy,inputLine,30*sizeof(int));
        int j = i;
        //copy inputline but remove that input
        while(inputLine[j] != 0){
            inputCopy[j] = inputCopy[j+1];
            j++;
        }
        if(isValid(inputCopy) == true){
            free(inputCopy);
            return true;
        }
        i++;
...
void importList(char *lineptr, int **list, FILE *fptr){
    ...
    while(fgets(lineptr, 30, fptr) > 0) {
        ...
        bool Validity = isValid(listElement);

        if(Validity == false) Validity = ProblemDampener(listElement);
        validCount += (Validity);
        printf("%d, %d", Validity, validCount);
        printf("\n");
